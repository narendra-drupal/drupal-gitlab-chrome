(async () => {
  ...
  const regex =
      /https:\/\/www\.drupal\.org\/project\/issues\/search\/.*/g;
  if (url.match(regex)) {
    ...
    const assignedFields = document.querySelectorAll('td.views-field-field-issue-assigned');
    let names = []
    if (assignedText) {
      names = assignedText.split(',').map(name => name.trim());
    }
    else {
      names = [...new Set([...assignedFields].map(assignedField => assignedField.innerText.trim()))];
    }
